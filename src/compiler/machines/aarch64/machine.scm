...
;; W^X executable-shadow delta; must agree with REGBLOCK_CC_DELTA in
;; microcode/cmpintmd/aarch64.h.  Zero on systems where the heap is
;; executable in place.
(define-integrable register-block/cc-delta-offset 14)
;; Start address of the executable shadow view; must agree with
;; REGBLOCK_CC_SHADOW_START.  Zero when the heap is executable in place
;; (which makes the "already biased?" range test always skip).
(define-integrable register-block/cc-shadow-start-offset 15)

