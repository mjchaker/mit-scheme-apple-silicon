...
(define (pop-return)
  ;; The continuation datum is a canonical heap address; add the W^X
  ;; executable-shadow delta (zero when the heap is executable in
  ;; place) to get the PC to return to.
  (LAP ,@(pop rlr)
       ,@(object->address rlr rlr)
       (LDR X ,regnum:scratch-0 ,reg:cc-delta)
       (ADD X ,rlr ,rlr ,regnum:scratch-0)
       (RET)))

;; A BL leaves an executable-view (shadow) PC in the link register.
;; When that PC is affixed with a type and pushed as a continuation
;; datum, it must first be normalized to a canonical heap address (the
;; representation every tagged compiled datum uses; pop-return re-adds
;; the delta on return, and the GC relocates canonical datums).  TEMP
;; must be free; delta is zero when the heap is executable in place.
(define (canonicalize-rlr temp)
  (LAP (LDR X ,temp ,reg:cc-delta)
       (SUB X ,rlr ,rlr ,temp)))
    ...
    (lambda (shared-label)
      (let ((interrupt-label (generate-label 'INTERRUPT)))
        (LAP (LABEL ,shared-label)
             ...
             ,@(object->address rlr rlr)
             ;; See pop-return: bias the canonical datum into the
             ;; executable shadow view.
             (LDR X ,regnum:scratch-0 ,reg:cc-delta)
             (ADD X ,rlr ,rlr ,regnum:scratch-0)
             (RET)
...
(define (entry->pc pc entry)
  ;; Compute PC = entry + stored offset, then bias into the executable
  ;; shadow view unless it is already there (W^X support).  Internal
  ;; entries reached by computed jump have plain compile-time offsets
  ;; (so entry+offset is canonical and must be biased); closure and
  ;; fasload-adjusted offsets already include the delta (so the range
  ;; test skips).  When the heap is executable in place, cc-shadow-start
  ;; and cc-delta are zero, so the unsigned compare against zero always
  ;; skips.  Uses scratch-0, which the sole caller (COMPUTED-JUMP) has
  ;; free at this point.
  (let ((no-bias (generate-label 'ALREADY-SHADOW)))
    (LAP (LDUR X ,pc (+ ,entry (& -8)))
         (ADD X ,pc ,pc ,entry)
         (LDR X ,regnum:scratch-0 ,reg:cc-shadow-start)
         (CMP X ,pc ,regnum:scratch-0)
         (B. HS (@PCR ,no-bias ,regnum:scratch-0))
         (LDR X ,regnum:scratch-0 ,reg:cc-delta)
         (ADD X ,pc ,pc ,regnum:scratch-0)
        (LABEL ,no-bias))))
...
(define (generate-closure-entry label padding min max offset temp)
  (let* ((label* (rtl-procedure/external-label (label->object label)))
    ...
    (LAP ,@(load-unsigned-immediate temp format)
         ...
         ;; ADR yields an executable-view (shadow) PC while Free is a
         ;; canonical heap address, so the difference carries the W^X
         ;; delta.  Remove it: every *stored* PC offset is delta-free,
         ;; which is what makes a dumped band independent of the delta
         ;; the kernel happens to give us.  Use sites re-add it.
         ;; (Zero, hence a no-op, where the heap is executable in place.)
         (LDR X ,regnum:scratch-0 ,reg:cc-delta)
         (SUB X ,temp ,temp ,regnum:scratch-0)
         ;; Store the PC offset.
         (STR X ,temp (POST+ ,Free (& 8))))))
...
(define (make-closure-padded-format padding min max offset)
  ...
  (let ((word
    ...
    ;; The shift by 32 is done as two shifts by 16 to work around a
    ;; code-generation bug in the 12.1 x86-64 compiler used as the
    ;; cross-compilation host: a checked left shift by n >= 32 is
    ;; open-coded as IMUL by 2^n, whose immediate operand must fit in
    ;; 32 bits, and the fallback emits an illegal three-register IMUL
    ;; (see machines/x86-64/rulfix.scm, load-fixnum-constant).
    (case endianness
      ((BIG) (bitwise-ior padding (shift-left (shift-left word 16) 16)))
      ((LITTLE) (bitwise-ior (shift-left (shift-left word 16) 16) padding))
...
(define (generate/quotation-header environment-label free-ref-label n-sections)
  (let ((continuation-label (generate-label 'LINKED)))
    ...
    ;; ADR yields an executable-view (shadow) PC; the store of the
    ;; environment must go through the writable view, so subtract the
    ;; W^X delta (zero when the heap is executable in place).  The
    ;; block and free-ref addresses passed to the linker are
    ;; normalized on the microcode side (comutil_link).
    (LAP (LDR X ,r0 ,reg:environment)
         (ADR X ,r1 (@PCR ,environment-label ,regnum:scratch-0))
         (LDR X ,regnum:scratch-0 ,reg:cc-delta)
         (SUB X ,r1 ,r1 ,regnum:scratch-0)
         (STR X ,r0 ,r1)
