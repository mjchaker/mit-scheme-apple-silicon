...
(define-rule statement
  ...
  ;; The PC-relative address is an executable (shadow-view) PC;
  ;; subtract the W^X shadow delta (zero when the heap is executable
  ;; in place) so the stored datum is a canonical heap address.
  (let ((target (standard-target! target)))
    (LAP ,@(load-pc-relative-address
            target
            (rtl-procedure/external-label (label->object label)))
         (LDR X ,regnum:scratch-0 ,reg:cc-delta)
         (SUB X ,target ,target ,regnum:scratch-0))))

(define-rule statement
  (ASSIGN (REGISTER (? target)) (ENTRY:CONTINUATION (? label)))
  (rtl-target:=machine-register! target regnum:link-register)
  (let ((linked (generate-label 'LINKED)))
    ;; BL leaves the shadow-view address of the B instruction in the
    ;; link register; normalize it to a canonical datum (see
    ;; ENTRY:PROCEDURE above).
    (LAP (BL (@PCR ,linked ,regnum:scratch-0))
         (B (@PCR ,label ,regnum:scratch-0))
        (LABEL ,linked)
         (LDR X ,regnum:scratch-0 ,reg:cc-delta)
         (SUB X ,regnum:link-register
              ,regnum:link-register
              ,regnum:scratch-0))))
