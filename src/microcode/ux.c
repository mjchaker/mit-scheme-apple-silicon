...
/* macOS on AArch64 refuses mappings that are simultaneously writable
   and executable, so there the heap is mapped writable only, and
   doubled with a read/execute "shadow" view of the same physical
   pages made with mach_vm_remap; compiled code runs in the shadow
   view.  See cmpint.h for the conventions that make this work.  */
#if defined(CC_IS_NATIVE) && defined(__APPLE__) && defined(__aarch64__)
#  define USE_CC_EXEC_SHADOW 1
#  include <mach/mach.h>
...
#ifdef USE_CC_EXEC_SHADOW

static bool
setup_cc_exec_shadow (void * addr, unsigned long size)
{
  vm_prot_t cur = VM_PROT_NONE;
  vm_prot_t max = VM_PROT_NONE;
  /* Prefer a fixed delta (4 GiB above the heap): a deterministic
     delta keeps bands dumped by this machine trivially rebasable and
     avoids any chance of a stale band's shadow range overlapping this
     process's (see FASLHDR_CC_DELTA).  Fall back to a kernel-chosen
     spot if that range is taken.  */
  mach_vm_address_t shadow
  ...
  if (kr != KERN_SUCCESS)
    return (false);
  /* The range test in aarch64_entry_pc / apply_setup assumes the
     shadow lies strictly above the canonical region.  */
  if (shadow < ((((mach_vm_address_t) addr)) + size))
    {
      (void) mach_vm_deallocate ((mach_task_self ()), shadow, size);
      return (false);
    }
  if ((mprotect (((void *) shadow), size, (PROT_READ | PROT_EXEC))) != 0)
    {
      (void) mach_vm_deallocate ((mach_task_self ()), shadow, size);
      return (false);
    }
  /* The offset arithmetic is modular, so it is fine for the shadow to
     be below the heap.  Both mappings are page-aligned, which keeps
     ADRP page computations consistent between the views.  */
  cc_exec_base = ((unsigned long) addr);
  cc_exec_size = size;
  cc_exec_delta = (((unsigned long) shadow) - ((unsigned long) addr));
  return (true);
}

#endif /* USE_CC_EXEC_SHADOW */
...
#endif
#ifdef USE_CC_EXEC_SHADOW
	  if (!setup_cc_exec_shadow (addr, request))
	    outf_error
	      ("unable to map executable shadow of heap"
	       " -- native code will fail\n");
#endif
	  return (addr);
	}
...
#if defined(CC_IS_NATIVE) && !defined(USE_CC_EXEC_SHADOW)
  /* With the exec shadow, the primary heap mapping must not (and on
     macOS/AArch64 cannot) be executable; execution happens in the
     read/execute shadow view instead.  */
  prot |= PROT_EXEC;
#endif
