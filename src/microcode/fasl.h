...
#define FASL_OFFSET_CC_DELTA	19	/* W^X shadow delta of dumper (bands) */

...
typedef struct
{
  ...
  /* W^X exec-shadow delta of the dumping process; nonzero only in
     bands (raw heap images) dumped with a dual-mapped heap.  The
     loader rebases compiled-entry PC offsets from this to its own
     delta.  */
  unsigned long cc_delta;
} fasl_header_t;
...
#define FASLHDR_CC_DELTA(h) ((h)->cc_delta)
