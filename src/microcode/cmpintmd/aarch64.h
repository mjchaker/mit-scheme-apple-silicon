...
/* Register-block slot holding cc_exec_delta for compiled code (used by
   pop-return and entry materialization).  Slots 0-13 are defined in
   const.h; COMPILER_REGBLOCK_N_FIXED is 16, so 14 is free.  Must agree
   with aarch64/lapgen.scm (reg:cc-delta).  */
#define REGBLOCK_CC_DELTA 14
...
/* PC = entry + stored offset, then biased into the executable shadow
   view if the offset did not already include the delta.  Offsets are
   biased at fasload for entries referenced as objects, and closure
   offsets carry the delta intrinsically, but entries materialized at
   run time from in-block labels have plain offsets, so the range
   test here (and its twin in cmpauxmd/aarch64.m4 apply_setup) is
   what makes every path land in the shadow.  */
#define CC_ENTRY_ADDRESS_PC(e) (aarch64_entry_pc (e))

extern insn_t * aarch64_entry_pc (insn_t *);

/* Register-block slot holding the shadow view's start address (zero
   when the heap is executable in place); used by apply_setup for the
   same range test.  Must agree with cmpauxmd/aarch64.m4.  */
#define REGBLOCK_CC_SHADOW_START 15

/* A compiled return address points to a jump instruction that jumps to
   the continuation's body.

   The PTR is handed to interface_to_scheme_return, which RETs straight
   to it, so it must be an executable (shadow-view) address; the datum
   itself is canonical.  CC_CANONICAL_TO_PC is the identity when
   cc_exec_delta is zero.  */

#define CC_RETURN_ADDRESS_PTR(r)	(CC_CANONICAL_TO_PC (r))
#define CC_RETURN_ADDRESS_PC(r)		((insn_t *) interface_to_scheme_return)

/* The 64-bit PC-offset field stored immediately before an entry, as a
   SCHEME_OBJECT-sized slot; used by fasload/fasdump to bias/unbias
   offsets by cc_exec_delta (see cmpint.h).  */

#define CC_ENTRY_PC_OFFSET_SLOT(e) (((SCHEME_OBJECT *) (e)) - 1)
...
/* Hook table layout; must match cmpauxmd/aarch64.m4 (and, for the
   indices generated code uses, DEFINE-ENTRIES in aarch64/lapgen.scm).
   hook_closure_apply is used only by write_uuo_target, so it sits
   past the end of the DEFINE-ENTRIES list.  */
#define HOOK_SIZE_IN_BYTES 16
#define HOOK_CLOSURE_APPLY_INDEX 0x1b

/* Machine registers referenced when assembling execute caches; must
   match the .req declarations in cmpauxmd/aarch64.m4.  */
#define AARCH64_HOOKS_REGNO 23
#define AARCH64_IP0_REGNO 16
