...
load_file (fasl_file_handle_t handle, unsigned long old_ephemeron_count)
{
  ...
  /* When the executable shadow is in use (cc_exec_delta nonzero), the
     relocation scan must always run: it is what biases the compiled
     entries' PC offsets for this process's delta, even if no address
...
/* Note: no W^X adjustment happens here.  Stored PC offsets are
   delta-free both on disk and in memory (see cmpint.h), so a loaded
   image needs no rebasing -- which is exactly what lets a band load
   under whatever executable-shadow delta the kernel gives this
   process, rather than only the one the dumping process had.  */
