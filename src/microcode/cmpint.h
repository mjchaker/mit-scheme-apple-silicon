...
/* W^X support.  On operating systems that forbid mappings that are
   simultaneously writable and executable (e.g. macOS on AArch64), the
   Scheme heap is mapped twice: writable at the canonical addresses
   used by all Scheme objects, and executable in a read-only "shadow"
   view cc_exec_delta bytes above.  Compiled code runs in the shadow
   view.  On all other systems cc_exec_delta is zero and these macros
   are no-ops.

   Conventions when cc_exec_delta is nonzero:

   - Tagged compiled-entry datums stored in Scheme objects are
     canonical (writable-view) addresses.

   - Raw PC values -- return addresses pushed on the Scheme stack by
     compiled code, and trap-time PCs from signal contexts -- are
     shadow addresses.  Normalize with CC_PC_TO_CANONICAL before
     treating one as a heap pointer.

   - Every *stored* PC-offset field is delta-free: entry address + PC
     offset is a canonical address, in memory and on disk alike.  Each
     site that turns an entry into an executable PC adds the delta,
     via a range test ("if it is not already in the shadow, add"):
     apply_setup (cmpauxmd/aarch64.m4), CC_ENTRY_ADDRESS_PC
     (cmpintmd/aarch64.c) and entry->pc (compiler rules3.scm).

     The range test is needed because it is the only thing that can
     distinguish the two provenances -- and keeping offsets delta-free
     is what makes a dumped band loadable under whatever delta the
     kernel gives the next process (mach_vm_remap cannot be relied on
     to reproduce a fixed delta: fixed placement fails with NO_SPACE
     in the regions we would want).  Consequently neither fasload nor
     fasdump rebases anything.

     Keeping this invariant costs one subtraction in two places, where
     an ADR-derived (shadow) PC is turned into a stored offset:
     generate-closure-entry (rules3.scm) and store_trampoline_insns.  */

extern unsigned long cc_exec_delta;
extern unsigned long cc_exec_base;  /* canonical start of dual-mapped region */
extern unsigned long cc_exec_size;  /* size of dual-mapped region; 0 if none */


#define CC_EXEC_SHADOW_P(addr)						\
  ((cc_exec_delta != 0)							\
   && ((((unsigned long) (addr)) - (cc_exec_base + cc_exec_delta))	\
       < cc_exec_size))

#define CC_PC_TO_CANONICAL(pc)						\
  ((CC_EXEC_SHADOW_P (pc))						\
   ? ((insn_t *) (((unsigned long) (pc)) - cc_exec_delta))		\
   : ((insn_t *) (pc)))

#define CC_CANONICAL_TO_PC(addr)					\
  ((insn_t *) (((unsigned long) (addr)) + cc_exec_delta))

