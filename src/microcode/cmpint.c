...
/* W^X dual-mapping state; see cmpint.h.  Nonzero only on systems that
   forbid write+execute mappings, where the OS layer (e.g. ux.c) sets
   these up when allocating the heap.  */
unsigned long cc_exec_delta = 0;
unsigned long cc_exec_base = 0;
unsigned long cc_exec_size = 0;
...
compiler_reset (SCHEME_OBJECT new_block)
{
  ...
  /* Re-store the trampoline instructions and PC offsets.  A restored
     band carries the trampolines exactly as the dumping session wrote
     them, including that session's cc_exec_delta bias in the PC
     offsets, which is stale in this process.  Rewriting them is cheap
     and makes the utilities block correct regardless of provenance.  */
  {
    cc_entry_type_t cet;
    make_cc_entry_type ((&cet), CET_RETURN_TO_INTERPRETER);
    if ((fill_trampoline (nbp, 0, (&cet), TRAMPOLINE_K_RETURN_TO_INTERPRETER))
	|| (fill_trampoline (nbp, 1, (&cet), TRAMPOLINE_K_REFLECT_TO_INTERFACE)))
      {
	outf_fatal ("\nError resetting compiler utilities\n");
	Microcode_Termination (TERM_COMPILER_DEATH);
	/*NOTREACHED*/
      }
#ifdef FLUSH_I_CACHE_REGION
    FLUSH_I_CACHE_REGION (nbp, n_words);
#endif
  }

#ifdef REGBLOCK_CC_DELTA
  /* Give compiled code access to the W^X shadow delta (zero when the
     heap is executable in place).  */
  (Registers[REGBLOCK_CC_DELTA]) = ((SCHEME_OBJECT) cc_exec_delta);
#endif
#ifdef REGBLOCK_CC_SHADOW_START
  /* Shadow start for apply_setup's range test; zero (test always
     skips) when the heap is executable in place.  */
  (Registers[REGBLOCK_CC_SHADOW_START])
    = ((SCHEME_OBJECT)
       ((cc_exec_delta != 0) ? (cc_exec_base + cc_exec_delta) : 0));
#endif
...
DEFINE_SCHEME_UTILITY_2 (comutil_lexpr_apply, address, n_args)
{
  ...
  /* Compiled code passes shadow-view addresses; normalize (no-op if
     cc_exec_delta is zero or the value is already canonical).  */
  address = (CC_PC_TO_CANONICAL (address));
			 ...
			 n_sections)
{
  ...
  /* Compiled code passes shadow-view addresses; normalize (no-op if
     cc_exec_delta is zero or the value is already canonical).  */
  return_addr = (CC_PC_TO_CANONICAL (return_addr));
  block_addr = ((SCHEME_OBJECT *) (CC_PC_TO_CANONICAL (block_addr)));
  constant_addr = ((SCHEME_OBJECT *) (CC_PC_TO_CANONICAL (constant_addr)));
			   ...
			   SCHEME_OBJECT state)
{
  ...
  STACK_CHECK (0);
  if (entry_address != 0)
    /* The entry address arrives raw from compiled code, i.e. as a
       shadow-view PC; the datum stored must be canonical.  */
    STACK_PUSH (MAKE_CC_ENTRY (CC_PC_TO_CANONICAL (entry_address)));
			 ...
			 ret_addr, cache_addr, new_val)
{
  ...
  /* Compiled code passes shadow-view addresses; normalize (no-op if
     cc_exec_delta is zero or the value is already canonical).  */
  ret_addr = (CC_PC_TO_CANONICAL (ret_addr));
  cache_addr = ((SCHEME_OBJECT *) (CC_PC_TO_CANONICAL (cache_addr)));
...
#define INIT_TRAMPOLINE_1(av1)						\
  ...
  TRAMP_store = ((SCHEME_OBJECT *) (CC_PC_TO_CANONICAL (TRAMP_store)));		\
...
DEFINE_SCHEME_UTILITY_2 (comutil_compiled_code_bkpt, entry_addr, state)
{
  ...
  /* Compiled code passes shadow-view addresses; normalize (no-op if
     cc_exec_delta is zero or the value is already canonical).  */
  entry_addr = (CC_PC_TO_CANONICAL (entry_addr));
