...
encode_fasl_header (SCHEME_OBJECT * raw, fasl_header_t * h)
{
  ...
  /* Recorded for diagnostics only: stored PC offsets are delta-free
     (see cmpint.h), so an image loads correctly under any delta and
     the loader does not consult this.  */
  (raw[FASL_OFFSET_CC_DELTA])
    = (MAKE_OBJECT (TC_BROKEN_HEART, cc_exec_delta));
}

static bool
decode_fasl_header (SCHEME_OBJECT * raw, fasl_header_t * h)
{
  ...
  }
  {
    ...
    /* Old writers fill unused header words with #F, so the type check
       distinguishes files that predate this field.  */
    (FASLHDR_CC_DELTA (h))
      = (((OBJECT_TYPE (raw[FASL_OFFSET_CC_DELTA])) == TC_BROKEN_HEART)
	 ? (OBJECT_DATUM (raw[FASL_OFFSET_CC_DELTA]))
	 : 0);
