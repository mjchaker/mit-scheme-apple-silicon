...
{
#ifdef CC_SUPPORT_P
  /* Compiled code executes in the shadow view when cc_exec_delta is
     nonzero, so a trap-time PC is a shadow address; normalize it to
     the canonical heap address before classifying.  */
  pc = ((unsigned long) (CC_PC_TO_CANONICAL (pc)));
  if (PC_ALIGNED_P (pc))
    {
