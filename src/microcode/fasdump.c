...
at by compiled code are ignored (and discarded).")
{
  ...
  /* Read this before the commit: the commit runs free_prim_renumber
     (registered with tat_always), so current_pr is dead after it.
     Reading it afterward worked by accident until allocators started
     zeroing freed memory (e.g. macOS 26).  */
  n_prims = (current_pr->next_code);
  ...
  (FASLHDR_N_PRIMITIVES (fh)) = n_prims;
...
DEFINE_GC_OBJECT_HANDLER (fasdump_cc_entry)
{
  ...
  /* No W^X adjustment: stored PC offsets are delta-free in memory as
     well as on disk (see cmpint.h), so the copy needs no un-biasing.  */
