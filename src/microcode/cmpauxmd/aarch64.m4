...
ifdef(`__APPLE__',
	...
	// W^X executable-shadow support; must agree with
	// cmpintmd/aarch64.h.  CC_DELTA is the shadow delta (zero when
	// the heap is executable in place); CC_SHADOW_START is the
	// shadow view's start address (zero when unused, which makes
	// the apply_setup range test always skip the bias).
	.equiv	REGBLOCK_CC_DELTA,	14
	.equiv	REGBLOCK_CC_SHADOW_START, 15
...
LOCAL(apply_setup)
	...
	// Bias the PC into the executable shadow view unless the
	// offset already included the delta (W^X support; the slot
	// holds zero when the heap is executable in place, which
	// makes the test always skip).
	ldr	ip0, [REGS, #(REGBLOCK_CC_SHADOW_START*8)]
	cmp	APPLICAND_PC, ip0
	b.hs	2f
	ldr	ip0, [REGS, #(REGBLOCK_CC_DELTA*8)]
	add	APPLICAND_PC, APPLICAND_PC, ip0
2:	ret
...
END(apply_setup_fail)
	...
	// closure_apply(entry@x1)
	//
	//	Jump into the compiled closure entry whose canonical
	//	(writable-view) address is in APPLICAND=x1.  Stored PC
	//	offsets are delta-free (see microcode/cmpint.h), so the
	//	W^X shadow delta has to be added to get the address to
	//	execute.  This lives in a hook because the execute cache
	//	that jumps here is exactly 32 bytes and has no room to
	//	inline the extra loads; see write_uuo_target.
	//
LOCAL(closure_apply)
	ldur	APPLICAND_PC, [APPLICAND, #-8]		// x17 := PC offset
	add	APPLICAND_PC, APPLICAND_PC, APPLICAND	// += entry
	// Bias into the executable shadow view unless already there.
	// (Both slots are zero where the heap is executable in place,
	// which makes the unsigned compare always skip.)
	ldr	ip0, [REGS, #(REGBLOCK_CC_SHADOW_START*8)]
	cmp	APPLICAND_PC, ip0
	b.hs	1f
	ldr	ip0, [REGS, #(REGBLOCK_CC_DELTA*8)]
...
LOCAL(hooks)
	...
	// Not referenced by generated code (so it need not appear in
	// DEFINE-ENTRIES); used by execute caches built by
	// write_uuo_target, which knows this index.
	JUMP_HOOK(hook_closure_apply, closure_apply)			// 1b
END(hooks)

// Local Variables:
// comment-start: "//"
